...
class TariffsAdapter(var context: Context, var list: ArrayList<TariffsModel>) : RecyclerView.Adapter<TariffsAdapter.MyViewHolder>() {

    inner class MyViewHolder(var itemView2: View) : RecyclerView.ViewHolder(itemView2) {
        fun onBind(model: TariffsModel, position: Int) {
            ...
            itemView2.setOnClickListener{
                ...
                itemView.findNavController().navigate(R.id.tariffsShowFragment, bundleOf("tariffName" to list[position].name, "tariffCode" to list[position].code, "tariffDesc" to list[position].desc), navOption.build())
            }
