    ...
    inner class MyViewHolder(var itemView3: View) : RecyclerView.ViewHolder(itemView3) {
        fun onBind(model: MinutesModel, position: Int) {
            ...
            itemView3.setOnClickListener{
                ...
                itemView.findNavController().navigate(R.id.minutesShowFragment, bundleOf("minutesMinutes" to list[position].minutes, "minutesCode" to list[position].code, "minutesDesc" to list[position].desc), navOption.build())
            }
